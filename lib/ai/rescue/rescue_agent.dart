...
class RescueAgent {
  ...
  Future<RecoveryPlan> generateRecoveryPlan(List<TaskEntity> tasks) async {
    ...
    for (var task in pendingTasks) {
      taskContext += "- [ID: ${task.taskId}] ${task.title} (Deadline: ${task.deadline}, Risk: ${task.riskScore ?? 0}%, Est. Duration: ${task.estimatedDuration} mins)\n";
    }
    ...
    final schema = Schema.object(
      properties: {
        ...
        'dropTaskIds': Schema.array(items: Schema.string(), description: 'The IDs of the tasks that should be delayed or dropped (lowest priority, highest effort, furthest deadline).'),
        ...
        'keepTaskIds': Schema.array(items: Schema.string(), description: 'The IDs of the tasks that MUST be kept because they are too high risk or important.'),
      },
...
Analyze the tasks and brutally decide which ones MUST be dropped or delayed to tomorrow to save the most critical, high-risk, near-deadline tasks today.
